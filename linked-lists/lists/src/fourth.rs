...
impl<T> List<T> {
    ...
    pub fn pop_front(&mut self) -> Option<T> {
        self.head.take().map(|old_head| {
            ...
            // into_inner() consumes the RefCell, returning the value inside
            Rc::try_unwrap(old_head).ok().unwrap().into_inner().elem
        })
    }
    pub fn peek_front(&self) -> Option<Ref<T>> {
        self.head.as_ref().map(|node| {
            // map over Ref and return a Ref to the element inside the RefCell
...
/// An iterator that yield elements from the other end of the list
